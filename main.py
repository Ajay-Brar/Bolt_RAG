...
def generate_answer_with_groq(question: str, context_chunks: List[str]) -> str:
    ...
    # Truncate to avoid exceeding model token limits (~30k TPM)
    max_chars = 15000
    if len(context) > max_chars:
        context = context[:max_chars] + "\n... [Content truncated due to length limits]"
    prompt = f"""You are a helpful and precise AI assistant. Your job is to answer questions using ONLY the CONTEXT provided below.
...
def ask_question(input_data: QuestionInput, db: Session = Depends(db.get_db_connection)):
    try:
        ...
        ##if not matched_ids or all(match.get("score", 0.0) < 0.35 for match in matches): "old code"
        if not matched_ids or all(match.get("score", 0.0) < 0.0 for match in matches):
